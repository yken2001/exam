...
年度選單裡的「AI-1」是 AI 出的一整個年度的練習卷（內部年度代碼 901），六科都有：國文 42、英文 43、英聽 21、數學 25、自然 50、社會 54 題，題數與題型比照會考。和會考真題分開標示「AI 題」，不評官方等級、只看正確率。

- **AI-1 英語（閱讀）**：43 題（單題 19 題＋題組 8 組 24 題，含克漏字），依心測中心英語科命題架構（單題：語言成分；題組：篇章理解）與「取材於學生學習及生活經驗」的原則，結合時事（太陽能海水淡化、巴布亞企鵝四物種、2026 花蓮國際少年運動會、學生使用 AI 等）。每題附中文翻譯與解析，每篇附文章翻譯。
- **數學、自然、社會**：每題正確選項放第一、數值答案由程式計算並比對（`tools/ai_sets/ai1_*.py`），選項位置由程式排列、解析的「故選」也由程式寫入；附圖（幾何、坐標、曲線圖、表格）由 matplotlib 依題目數據繪製（`public/ai/901/`）。時事素材先查證（例：2026 國際少年運動會）。
- **英聽**：辨識句意 3 題（程式繪製的三格圖）、基本問答 8 題、言談理解 10 題；語音由 `python tools/gen_audio.py ai1` 依聽力稿合成。
- **國文**：文言文逐字取自維基文庫（《世說新語》〈德行〉〈方正〉、《論語·學而》），查不到可靠原文的篇章不採用；白話文章為原創；不出字音（注音聲調）題。
- **題目檔**：`src/data/ai/ai1_english.json`（文字題：文章、題目、選項、答案、解析；表格用 `| a | b |`，克漏字空格用 `__35__`）。
- **把關流程**：
  1. `python tools/ai_booklet.py <題目檔> <輸出>` 產生不含答案的題本（附圖以圖檔路徑提供、英聽附聽力稿），交給兩個不同模型的 AI **各自獨立作答**，兩者答案都必須和答案卷完全一致，並指出有兩個答案、題意不清或文法錯誤的題目。
  2. 有被指出的題目就改寫，改寫後的題目再交給全新的 AI 重新作答，直到沒有異議；最後用定稿版本再完整作答一次。
  3. `python tools/check_ai_set.py` 檢查題號、選項、題組範圍、克漏字空格，以及每題解析的「故選 (X)」必須等於答案。
  4. 驗證紀錄寫在題目檔的 `verification` 欄位。
