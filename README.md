...
只用每科的「**xxx會考xx解析.pdf**」（題本不需要），110–113 年每年 5 份，113 年另有單獨的「113會考英聽解析.pdf」，共 21 份：
...
| **合計** | 174 | 170 | 42 | 101 | 204 | 225 | **916** |

- 只收選擇題：數學的非選擇題（手寫題）不收；國文解析卷沒有作文。
- 英聽：112 年收在英文解析卷前段，113 年是單獨一份英聽解析卷；110、111 年的英文解析卷只有閱讀，沒有英聽。
...
flowchart LR
  subgraph SRC["來源（不進 git）"]
    PDF["21 份 解析 PDF<br/>tools/source_pdfs/"]
  end
  ...
  subgraph TOOLS["tools/（Python：PyMuPDF + Pillow + numpy）"]
    ...
    L["build_listening.py<br/>112、113 英聽各 21 題 + 錄音稿"]
...
6. **題組**：題目圖＝文章＋所有小題；110、111、113 每小題各有解析，112 則是整組一段「答案：(1)(C)；(2)(D)」，組內共用一張解析圖。
7. **英聽**（`tools/build_listening.py 112|113`）：112 是官方格式（「一、辨識句意」、題號「1.」、紅字「答案:(C)」「錄音稿:」）；113 是翰林格式（「第一部分:辨識句意」、題號「( C )第1題」、紅字【聽力稿】【中譯】【試題解析】…故選【C】）。錄音稿依 W:／M:／Question: 分段後交給 `gen_audio.py` 合成語音。
...
# 把 21 份「xxx會考xx解析.pdf」（含 113會考英聽解析.pdf）放到 tools/source_pdfs/
python tools/build_bank.py        # 重組 + 切割 + 驗證 + 輸出圖片（--dry 只看驗證報告）
python tools/build_listening.py 112   # 英聽（113 同樣跑一次）
python tools/gen_audio.py 113         # 英聽語音（需 Windows 的英文語音 Microsoft Zira；只有錄音稿變了才需要重跑）
